...
import ProjectsDetailForm from "./Form/ProjectsDetailForm";
...
const EditResume = () => {
  ...
  const validateAndNext = () => {
    ...
    switch (currentPage) {
      case "profile-info": {
        const fullName = resumeData?.profileInfo?.fullName ?? "";
        const designation = resumeData?.profileInfo?.designation ?? "";
        const summary = resumeData?.profileInfo?.summary ?? "";
      ...
      case "education-info":
        (resumeData.education || []).forEach(
          ({ degree, institution, startDate, endDate }, index) => {
            ...
            if (!institution.trim())
              errors.push(`Institute is required in education ${index + 1}`);
            if (!startDate || !endDate)
              errors.push(
  ...
  const uploadResumeImages = async () => {
    try {
      ...
      const imageDetaUrl = await captureElementAsImage(resumeRef.current);
